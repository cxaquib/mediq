...
class _LabTestRequestsScreenState extends State<LabTestRequestsScreen>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: TabBarView(
        controller: _tabController,
        children: List.generate(tabs.length, (_) => _buildTabContent()),
      ),
    );
  }
        ...
        return Card(
          child: ExpansionTile(
            ...
            subtitle: Text('Dr. ${[
              ...
              'Johnson',
              'Williams'
            ][index % 3]} • ${DateTime.now().subtract(Duration(days: index % 30)).toString().split(' ')[0]}'),
            trailing: Chip(
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _DetailRow(
                        label: 'Priority',
                        value: ['Normal', 'Urgent', 'Stat'][index % 3]),
                    _DetailRow(
                        label: 'Sample Collected',
                        value: index % 3 != 0 ? 'Yes' : 'Pending'),
                        ...
                        Expanded(
                            child: CustomButton(
                                text: status == 'Pending'
                                    ? 'Start Processing'
                                    : 'View Details',
                                isOutlined: status != 'Pending',
                                onPressed: () {})),
                        const SizedBox(width: 12),
                        if (status == 'Processing')
                          Expanded(
                              child: CustomButton(
                                  text: 'Mark Complete', onPressed: () {})),
                      ],
                    ),
                  ],
                ),
